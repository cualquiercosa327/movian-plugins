...
(function(plugin) {
    ...
    //settings.createString('baseURL', "Base URL without '/' at the end", 'https://www.popcorntime.ws/api/eztv', function(v) {
    //settings.createString('baseURL', "Base URL without '/' at the end", 'https://popcorntimece.ch/api/v2', function(v) {
    //settings.createString('baseURL', "Base URL without '/' at the end", 'https://popcorntimece.ch/api/eztv', function(v) {
    settings.createString('baseURL', "Base URL without '/' at the end", 'https://eztvapi.ml', function(v) {
        service.baseUrl = v;
    });
    ...
    function browseItems(page, query) {
        ...
        function loader() {
            if (!offsetje) return false;
            page.loading = true;
            if (!service.sortby)
                service.sortby = '';
            var url = service.baseUrl + '/shows/' + offsetje + service.sortby;
            if (query) {
            ...
            page.loading = false;
            if (doc[0] != '[') return offsetje = false;
            var json = showtime.JSONDecode(doc);
            if (offsetje == 1 && page.metadata)
               page.metadata.title
    ...
    plugin.addURI(plugin.getDescriptor().id + ":showlist:(.*)", function browseshowItems(page, showid) {
		...
		page.metadata.logo = cde.images.poster;
        ...
        for (var i in episodes) {
	        ...
	        icon: cde.images.poster,
	            ...
	            coloredStr(' Episode: ', orange) + episodes[i].episode +
					...
					coloredStr(' Status: ', orange) + cde.status +
